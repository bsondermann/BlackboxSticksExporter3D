  ...
  void show(){
    ...
    for(int i = 0; i< renderers.length;i++){
      ...
      if(renderers[i].running){
        ...
        textAlign(RIGHT,TOP);
        text(renderers[i].currentState,width-140,posy+posindex*50+offset);
        ...
        text(renderers[i].getRuntime()/60+":"+sec1+"    |    "+sec21+sec2,(width-140)/2,posy+posindex*50+offset);
        
        noFill();
        strokeWeight(3);
        stroke(0);
        rect(0,posy+posindex*50+offset+20,width-140,20);
        fill(255,0,0);
        rect(0,posy+posindex*50+offset+20,map(constrain(renderers[i].currentFrame,0,renderers[i].numFrames),0,renderers[i].numFrames,0,width-140),20);
        strokeWeight(1);
        fill(150);
        stroke(0);
        textAlign(CENTER,TOP);
        text((int)map(constrain(renderers[i].currentFrame,0,renderers[i].numFrames),0,renderers[i].numFrames,0,100)+"%",(width-140)/2,posy+posindex*50+offset+24);
        fill(50);
        strokeWeight(3);
        rect(width-137,posy+posindex*50+offset,120,40);
        strokeWeight(1);
        imageMode(CORNER);
        if(renderers[i].prevImage!=null){
          image(renderers[i].prevImage,width-137,posy+posindex*50+offset+2,120,40);
        }
        posindex++;
      }
